...
class Program
{
    ...
    {
        Console.Write($"Player {player}, choose a name: ");
    ...
    public static void SetUpDefaultGame(out Player player1, out Player player2, out HexGrid grid)
    {
        ...
        int gridSize = 8;
        grid = new HexGrid(gridSize);
        player1 = new Player(GetPlayerName("One"), 0, 10, 10, 5);
        player2 = new Player(GetPlayerName("Two"), 1, 10, 10, 5);
        grid.SetUpGridTerrain(t);
    ...
    public static void PlayGame(Player player1, Player player2, HexGrid grid)
    {
        ...
        do
        {
            ...
            {
                var str = c.Trim().Split(' ');
                List<string> items = new List<string>(str);
            ...
            if (gameOver)
            {
                grid.DestroyPiecesAndCountVPs(ref player1, ref player2);
            }
            else
                gameOver = grid.DestroyPiecesAndCountVPs(ref player1, ref player2);
            Console.WriteLine("Player One current state - " + player1.GetStateString());
            Console.WriteLine("Player Two current state - " + player2.GetStateString());
    ...
    public bool DestroyPiecesAndCountVPs(ref Player player1, ref Player player2)
    {
            ...
            if (t.GetPieceInTile() != null)
            {
                ...
                if (noOfConnections >= thePiece.GetConnectionsNeededToDestroy())
                {
                    ...
                    if (thePiece.GetBelongsToPlayer1())
                    {
                        ...
                        player2.AddToVPs(thePiece.GetVPs());
                    }
                    else
                    {
                        player2.AddPiecesInSupply(1);
                        player1.AddToVPs(thePiece.GetVPs());
                    }
                }
            }
        }
